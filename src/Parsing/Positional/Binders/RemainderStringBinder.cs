...
{
    internal sealed class RemainderStringBinder : IParameterBinder<string>
    {
        ...
        public string Bind(IParameter parameter, ReadOnlySpan<char> text,
            out bool success)
        {
            if (parameter is not IRemainderParameter &&
                (IsStandardQuoted(text) || IsQuotePair(text[1], text[^1])))
            {
                text = text[1..^1];
            }
